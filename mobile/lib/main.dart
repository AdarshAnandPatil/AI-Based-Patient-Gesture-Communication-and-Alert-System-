...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> speakAlert(dynamic x) async {
    final code = x['language'] == 'kn' ? 'kn-IN' : x['language'] == 'hi' ? 'hi-IN' : 'en-IN';
    final message = '${x['message'] ?? ''}';
    final room = '${x['room'] ?? '-'}';
    final bed = '${x['bed'] ?? '-'}';
    // Keep the patient message in its original language. Android must have the
    // corresponding Kannada/Hindi TTS voice installed for native playback.
    final suffix = x['language'] == 'kn'
        ? ' ಕೊಠಡಿ $room. ಹಾಸಿಗೆ $bed.'
        : x['language'] == 'hi'
            ? ' कमरा $room. बिस्तर $bed.'
            : ' Room $room. Bed $bed.';
    try {
      await tts.stop();
      await tts.setLanguage(code);
      await tts.setSpeechRate(0.45);
      await tts.speak('$message.$suffix');
    } catch (_) {
      // Do not silently translate the patient message to English.
    }
  }
