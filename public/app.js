let state={alerts:[],reports:[],appointments:[]},filter="all",activeAlert=null,camera=null,alertReturnPage="gesture";
...
function overlay(raw){
 ...
 if(activePage && activePage!=="dashboard")alertReturnPage=activePage;
 activeAlert=a;
...
function closeAlertOverlay(){
 ...
 if(alertReturnPage && current!==alertReturnPage && alertReturnPage!=="dashboard")page(alertReturnPage);
}
  ...
  if(duration>=55&&duration<900){
   ...
   if(now-st.lastBlinkEventAt>900){
    ...
    if(n===1){setTimeout(()=>{
      ...
      if(cur.blinkTimes.length===1&&n2-cur.blinkTimes[0]>=700&&n2-cur.lastBlinkEventAt>800){cur.lastBlinkEventAt=n2;cur.blinkTimes=[];cur.pendingBlink1=true}
    },1100)}
   }
  }
 }
...
function handMotionEvent(st,lm,number,now){
 ...
 const point={x:wrist.x,y:wrist.y,z:wrist.z||0,open:openPalm,number,at:now};st.handHistory.push(point);st.handHistory=st.handHistory.filter(p=>now-p.at<1800);
 if(st.handHistory.length<7)return null;
 const recent=st.handHistory.slice(-Math.min(16,st.handHistory.length)),first=recent[0],last=recent[recent.length-1];
 const duration=last.at-first.at; if(duration<500)return null;
 ...
 if(now-st.lastHandEmergencyAt<700)return null;
 // Open palm moving toward/away from camera = breathing difficulty.
 if(nowOpen&&zSpan>.14&&duration>=650){st.lastHandEmergencyAt=now;return {key:"breathing",name:"Emergency Action — Difficulty Breathing",priority:"Critical",emoji:"🫁"}}
 // Open palm with a sharp downward movement = fall/injury signal.
 if(nowOpen&&dy>.22&&Math.abs(dy)>Math.abs(dx)*1.4&&duration>=600){st.lastHandEmergencyAt=now;return {key:"fall",name:"Emergency Action — Fall / Injury",priority:"Critical",emoji:"🩹"}}
 // Rapid left/right waving = sudden worsening.
 let changes=0;for(let i=1;i<recent.length;i++){const a=recent[i-1].x,b=recent[i].x;if(Math.abs(b-a)>.045)changes++}if(nowOpen&&changes>=5&&maxX>.25&&duration>=700){st.lastHandEmergencyAt=now;return {key:"worsening",name:"Emergency Action — Sudden Worsening",priority:"Critical",emoji:"⚠️"}}
 // Repeated fist movement in/out of the center = severe/sudden pain signal.
 if(fist&&recent.filter(p=>p.number===0).length>=6&&maxY>.18&&duration>=650){st.lastHandEmergencyAt=now;return {key:"pain",name:"Emergency Action — Severe / Sudden Pain",priority:"Critical",emoji:"😣"}}
 // Raised open palm held high for 2 seconds = immediate nurse/doctor.
 if(openPalm&&last.y<.30&&recent.filter(p=>p.open).length>=10&&duration>=900){st.lastHandEmergencyAt=now;return {key:"nurse",name:"Emergency Action — Immediate Nurse / Doctor",priority:"Critical",emoji:"👩‍⚕️"}}
 return null;
}
...
function drawMultiScene(){const c=$("outputCanvas"),ctx=c?.getContext("2d"),v=$("inputVideo");if(!ctx||!v)return;const w=v.videoWidth||640,h=v.videoHeight||480;if(c.width!==w)c.width=w;if(c.height!==h)c.height=h;ctx.clearRect(0,0,w,h);const zones=[{b:1,x:0,y:0},{b:2,x:.5,y:0},{b:3,x:0,y:.5},{b:4,x:.5,y:.5}];ctx.lineWidth=3;zones.forEach(z=>{ctx.strokeStyle="rgba(255,255,255,.92)";ctx.strokeRect(z.x*w,z.y*h,w/2,h/2);const cfg=getBedConfig(z.b);ctx.fillStyle="rgba(15,24,39,.82)";ctx.fillRect(z.x*w+7,z.y*h+7,250,30);ctx.fillStyle="#fff";ctx.font="bold 15px Arial";ctx.fillText(`BED ${z.b} · ${cfg.id} · ${cfg.name}`,z.x*w+14,z.y*h+28)});if(recognitionMode==="hand")latestHands.forEach(lm=>{const p=lm[0],bed=bedForPoint(p.x,p.y),cfg=getBedConfig(bed),pts=lm.map(q=>({x:q.x*w,y:q.y*h}));ctx.strokeStyle="#20d46b";ctx.fillStyle="#20d46b";ctx.lineWidth=4;HAND_CONNECTIONS.forEach(([a,b])=>{ctx.beginPath();ctx.moveTo(pts[a].x,pts[a].y);ctx.lineTo(pts[b].x,pts[b].y);ctx.stroke()});pts.forEach(q=>{ctx.beginPath();ctx.arc(q.x,q.y,4,0,Math.PI*2);ctx.fill()});const minX=Math.min(...pts.map(q=>q.x)),maxX=Math.max(...pts.map(q=>q.x)),minY=Math.min(...pts.map(q=>q.y)),maxY=Math.max(...pts.map(q=>q.y));ctx.strokeStyle="#fff";ctx.lineWidth=3;ctx.strokeRect(minX-8,minY-8,maxX-minX+16,maxY-minY+16);ctx.fillStyle="rgba(15,24,39,.84)";ctx.fillRect(Math.max(4,minX),Math.max(35,minY-31),275,26);ctx.fillStyle="#fff";ctx.font="bold 14px Arial";ctx.fillText(`${cfg.id} · Bed ${bed} · ${countFingers(lm)} fingers`,Math.max(10,minX+6),Math.max(52,minY-12))});else latestFaces.forEach(lm=>{const b=faceBox(lm),bed=bedForPoint(b.cx,b.cy),cfg=getBedConfig(bed);ctx.strokeStyle=recognitionMode==="eye"?"#ff9f43":recognitionMode==="head"?"#00b894":"#7c4dff";ctx.lineWidth=4;ctx.strokeRect(b.minX*w,b.minY*h,b.w*w,b.h*h);ctx.fillStyle=ctx.strokeStyle;ctx.font="bold 15px Arial";ctx.fillText(`${cfg.id} · Bed ${bed} · ${recognitionMode.toUpperCase()}`,b.minX*w,Math.max(18,b.minY*h-5))})}
function buildAlertVoice(message,a){
 const lang=String(a.language||"en").split("-")[0];
 if(lang==="kn")return `${message}. ರೋಗಿ ಸಂಖ್ಯೆ ${a.patientId}, ಕೊಠಡಿ ಸಂಖ್ಯೆ ${a.room}, ಬೆಡ್ ಸಂಖ್ಯೆ ${a.bed}.`;
 if(lang==="hi")return `${message}. मरीज संख्या ${a.patientId}, कमरा नंबर ${a.room}, बेड नंबर ${a.bed}.`;
 return `${message}. Patient ${a.patientId}, Room ${a.room}, Bed ${a.bed}.`;
}
...
async function createMultiAlert(cfg,gesture,message,priority="Normal",confidence=92,voiceText=message){
 ...
 st.lastAlertKey=key;st.lastAlertAt=now;st.armed=false;st.cooldownUntil=now+350;
 ...
 speak(buildAlertVoice(voiceText||payload.message,payload),payload.language);
 try{
...
function processHandForPatient(h,cfg,st,now){
 ...
 st.gestureHistory.push(number);if(st.gestureHistory.length>5)st.gestureHistory.shift();const freq={};st.gestureHistory.forEach(n=>freq[n]=(freq[n]||0)+1);const top=Object.entries(freq).sort((a,b)=>b[1]-a[1])[0];if(!top||+top[1]<4)return false;
 const detected=+top[0];if(st.gestureCandidate!==detected){st.gestureCandidate=detected;st.gestureSince=now;return false}if(now-st.gestureSince<280)return false;const g=gestureMap[detected],tx=labels[cfg.lang]||labels.en;
...
function drawSingleScene(r){const c=$("outputCanvas"),ctx=c?.getContext("2d"),v=$("inputVideo");if(!ctx||!v)return;const w=v.videoWidth||640,h=v.videoHeight||480;if(c.width!==w)c.width=w;if(c.height!==h)c.height=h;ctx.clearRect(0,0,w,h);const lm=r.multiHandLandmarks?.[0];if(lm&&recognitionMode==="hand"){const cfg=patientCfgSingle(),st=multiStates.single,now=Date.now(),pts=lm.map(p=>({x:p.x*w,y:p.y*h}));ctx.strokeStyle="#20d46b";ctx.fillStyle="#20d46b";ctx.lineWidth=4;HAND_CONNECTIONS.forEach(([a,b])=>{ctx.beginPath();ctx.moveTo(pts[a].x,pts[a].y);ctx.lineTo(pts[b].x,pts[b].y);ctx.stroke()});pts.forEach(p=>{ctx.beginPath();ctx.arc(p.x,p.y,5,0,Math.PI*2);ctx.fill()});const minX=Math.min(...pts.map(p=>p.x)),maxX=Math.max(...pts.map(p=>p.x)),minY=Math.min(...pts.map(p=>p.y)),maxY=Math.max(...pts.map(p=>p.y));ctx.strokeStyle="#fff";ctx.lineWidth=3;ctx.strokeRect(Math.max(2,minX-10),Math.max(2,minY-10),Math.min(w-4,maxX-minX+20),Math.min(h-4,maxY-minY+20));ctx.fillStyle="rgba(15,24,39,.82)";ctx.fillRect(Math.max(4,minX),Math.max(35,minY-32),300,27);ctx.fillStyle="#fff";ctx.font="bold 15px Arial";ctx.fillText(`${cfg.id} · Bed ${cfg.bed} · ${countFingers(lm)} fingers`,Math.max(10,minX+6),Math.max(52,minY-13));processHandForPatient(lm,cfg,st,now);$("cameraHint")?.classList.add("hidden");$("cameraStatus").textContent="✋ Hand detected"}else if(recognitionMode==="hand"){$("cameraStatus").textContent="AI ready — show one hand"}}
...
async function processVideoFrame(now){if(!cameraRunning)return;frameHandle=requestAnimationFrame(processVideoFrame);const v=$("inputVideo");if(v.readyState<2||processingFrame)return;if(now-lastProcessTime<100)return;lastProcessTime=now;processingFrame=true;try{if(recognitionMode==="hand"){await hands.send({image:v});if(patientMode==="multi"){if(!latestHands.length)for(let i=1;i<=4;i++)rearmState(multiStates[i],now,true);processMultiHands();drawMultiScene()}else if(!latestHands.length)rearmState(multiStates.single,now,true)}else{await faceMesh.send({image:v});if(patientMode==="multi"){if(!latestFaces.length)for(let i=1;i<=4;i++)rearmState(multiStates[i],now,true);processMultiFaces();drawMultiScene()}else if(latestFaces[0])processFaceForPatient(latestFaces[0],patientCfgSingle(),multiStates.single,now);else rearmState(multiStates.single,now,true);if(latestFaces[0])drawSingleFaceScene()}if(patientMode==="multi"){if(recognitionMode==="hand")$("cameraStatus").textContent=latestHands.length?`AI running — ${latestHands.length} hand${latestHands.length===1?"":"s"} detected`:"AI running — waiting for hands";else $("cameraStatus").textContent=latestFaces.length?`AI running — ${latestFaces.length} patient${latestFaces.length===1?"":"s"} detected`:"AI running — waiting for patients"}else $("cameraStatus").textContent=recognitionMode==="hand"?(latestHands.length?"✋ Hand detected":"AI ready — show one hand"):latestFaces.length?`AI running — ${recognitionMode} recognition`:"AI ready — show face/eyes"}catch(e){console.error("AI frame error",e);setCameraDiagnostic("AI frame error: "+e.message)}finally{processingFrame=false}}
async function startCamera(){try{if(cameraRunning)return;setCameraDiagnostic("Checking browser camera and AI libraries…");if(!window.isSecureContext)throw new Error("Camera requires HTTPS or localhost. Open the Render HTTPS address.");if(!navigator.mediaDevices?.getUserMedia)throw new Error("Camera API is unavailable in this browser. Use Chrome over HTTPS.");if(recognitionMode==="hand")await ensureMediaPipeHands();else await ensureFaceMesh();const v=$("inputVideo");if(!v)throw new Error("Camera video element was not found");if(recognitionMode==="hand"){hands=new window.Hands({locateFile:f=>mediaPipeBase+f});hands.setOptions({maxNumHands:patientMode==="multi"?4:1,modelComplexity:1,staticImageMode:false,minDetectionConfidence:.58,minTrackingConfidence:.58});hands.onResults(patientMode==="multi"?handleMultiHandsResults:drawSingleScene)}else{faceMesh=new window.FaceMesh({locateFile:f=>faceMeshBase+f});faceMesh.setOptions({maxNumFaces:patientMode==="multi"?4:1,refineLandmarks:true,minDetectionConfidence:.58,minTrackingConfidence:.58});faceMesh.onResults(handleMultiFaceResults)}resetMultiStates();multiStates.single=makePatientState();latestHands=[];latestFaces=[];cameraStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:1280,max:1280},height:{ideal:720,max:720},frameRate:{ideal:15,max:20}},audio:false});v.srcObject=cameraStream;v.muted=true;v.setAttribute("playsinline","");await v.play();cameraRunning=true;lastProcessTime=0;processingFrame=false;primeVoice();$("aiBadge").textContent=`${patientMode==="multi"?"MULTI-PATIENT":"SINGLE-PATIENT"} · ${recognitionMode.toUpperCase()} AI`;$('aiBadge').classList.add("running");$("cameraHint")?.classList.add("hidden");$("cameraHint").textContent=patientMode==="multi"?"Place patients inside their assigned zones and demonstrate one communication action at a time.":recognitionMode==="hand"?"Show one hand, hold the gesture briefly, then lower your hand before the next detection.":recognitionMode==="eye"?"Show your face, perform one eye action, wait for the result, then prepare the next action.":"Show your face, perform one head action, wait for the result, then prepare the next action.";setCameraDiagnostic(`✓ Camera connected. ${patientMode==="multi"?"Up to 4 patients":"One patient"} · ${recognitionMode} recognition active.`,true);$("multiAiStatus").textContent=`${patientMode==="multi"?"Multi-patient":"Single-patient"} ${recognitionMode} recognition is ready.`;frameHandle=requestAnimationFrame(processVideoFrame)}catch(e){console.error("Camera start failed",e);stopCamera();let msg=e.message||"Camera could not start";if(e.name==="NotAllowedError")msg="Camera permission denied. Allow Camera for this site in Chrome settings, then press Start again.";else if(e.name==="NotFoundError")msg="No camera found on this device.";else if(e.name==="NotReadableError")msg="Camera is busy or being used by another application.";else if(e.name==="SecurityError")msg="Camera blocked by browser security. Use the HTTPS Render address.";setCameraDiagnostic("⚠ "+msg);toast(msg)}}
function stopCamera(){cameraRunning=false;if(frameHandle)cancelAnimationFrame(frameHandle);frameHandle=null;processingFrame=false;if(cameraStream){cameraStream.getTracks().forEach(t=>t.stop());cameraStream=null}const v=$("inputVideo");if(v)v.srcObject=null;if(hands){try{hands.close?.()}catch{}hands=null}if(faceMesh){try{faceMesh.close?.()}catch{}faceMesh=null}latestHands=[];latestFaces=[];$("cameraHint")?.classList.remove("hidden");$("cameraStatus").textContent="Camera is off";$("aiBadge").textContent="AI READY";$("aiBadge").classList.remove("running");resetMultiStates();multiStates.single=makePatientState();$("detectedNeed").textContent="Ready for the next intentional communication action";$("multiAiStatus").textContent=`${patientMode==="multi"?"Multi-patient":"Single-patient"} ${recognitionMode} recognition is stopped.`}
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 $("alertOverlay").addEventListener("click",e=>{e.stopPropagation();if(e.target===$("alertOverlay"))closeAlertOverlay()},true);document.addEventListener("keydown",e=>{if(e.key==="Escape"&&$("alertOverlay").classList.contains("show"))closeAlertOverlay()});
 $("overlayVoiceBtn").addEventListener("click",(e)=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(activeAlert){const a=normalizeAlert(activeAlert);speak(buildAlertVoice(a.message,a),a.language)}},true);
 $("overlayAckBtn").addEventListener("click",async(e)=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();const a=activeAlert?normalizeAlert(activeAlert):null;if(!a)return;closeAlertOverlay();if(!String(a.id).startsWith("LOCAL-")){try{await act(a.id,"acknowledge")}catch{} }},true);
 $("overlayResolveBtn").addEventListener("click",async(e)=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();const a=activeAlert?normalizeAlert(activeAlert):null;if(!a)return;closeAlertOverlay();if(!String(a.id).startsWith("LOCAL-")){try{await act(a.id,"resolve")}catch{} }},true);
