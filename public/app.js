...
async function createGestureAlert(number){
 ...
 const payload={patientId:$("patientId").value||"P1001",patientName:"Demo Patient",room:$("room").value||"204",bed:$("bed").value||"3",gesture:g.name,message,language:l,priority:g.priority,confidence:95};
 try{
  const a=await api("/api/alerts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
  // Update the screen immediately with the alert returned by the server.
  ...
  toast("Gesture detected, but alert could not be saved. Check server connection.");
 }
}
