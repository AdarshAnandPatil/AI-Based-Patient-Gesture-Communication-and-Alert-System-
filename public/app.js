...
async function createMultiAlert(cfg,gesture,message,priority="Normal",confidence=92,voiceText=message){
 ...
 st.lastAlertKey=key;st.lastAlertAt=now;st.armed=false;st.cooldownUntil=now+5000;
...
function rearmState(st,now,signalAbsent=false){if(st.armed)return;if(now>=(st.cooldownUntil||0)&&signalAbsent){st.armed=true;st.gestureHistory=[];st.gestureCandidate=null;st.gestureSince=0;st.lastHeadSeq=[];st.headRepeatCount=0;st.blinkTimes=[];st.pendingBlink1=false;st.handHistory=[];}}
function processHandForPatient(h,cfg,st,now){
 rearmState(st,now,false);if(!st.armed)return false;const number=countFingers(h),motion=handMotionEvent(st,h,number,now);
...
function processFaceForPatient(lm,cfg,st,now){
 rearmState(st,now,false);if(!st.armed)return;const box=faceBox(lm),tx=multiText[cfg.lang]||multiText.en;
 if(recognitionMode==="eye"){const event=updateEyeState(st,eyeOpenScore(lm),now);if(event){if(event.kind==="blink3")return createMultiAlert(cfg,"3 Blinks — Emergency",tx.blink3,"Critical",94,tx.blink3);if(event.kind==="blink2")return createMultiAlert(cfg,"2 Blinks — Need Help",tx.blink2,"High",93,tx.blink2);if(event.kind==="blinkRapid")return createMultiAlert(cfg,"Rapid Repeated Blinking — Attention",tx.blinkRapid,"High",91,tx.blinkRapid);if(event.kind==="long")return createMultiAlert(cfg,"Prolonged Eye Closure",tx.long,"High",90,tx.long)}if(st.pendingBlink1){st.pendingBlink1=false;return createMultiAlert(cfg,"1 Blink — Yes / OK",tx.blink1,"Normal",90,tx.blink1)}return;}
 if(recognitionMode==="head"){const dir=getHeadDirection(lm,box,st);if(dir&&now-st.lastHeadAt>650){const same=dir===st.lastHeadDir;st.headRepeatCount=same?(st.headRepeatCount||1)+1:1;st.lastHeadAt=now;st.lastHeadDir=dir;const key=headActionForState(st,dir,now),priority=key==="headEmergency"?"Critical":(key==="headAttention"||key==="headWorse"||key==="headMonitoring")?"High":"Normal";return createMultiAlert(cfg,`Head Action — ${key}`,tx[key],priority,key==="headEmergency"?97:88,tx[key])}if(!dir&&now-st.lastHeadAt>1800){st.lastHeadDir=null;st.headRepeatCount=0}return;}
 ...
 const smile=(centerY-cornerAvgY)/mouthWidth>.10,frown=(cornerAvgY-centerY)/mouthWidth>.13,faceSignal=mouthOpen>.36?"breathing":smile?"smile":frown?"frown":"monitoring";
 if(faceSignal!==st.faceSignal){st.faceSignal=faceSignal;st.faceSignalSince=now;return}if(now-st.faceSignalSince>1200){const key=faceSignal==="breathing"?"faceBreathing":faceSignal==="smile"?"faceSmile":faceSignal==="frown"?"faceFrown":"faceMonitoring",priority=key==="faceBreathing"?"High":"Normal";return createMultiAlert(cfg,`Face — ${key}`,tx[key],priority,key==="faceBreathing"?86:80,tx[key]);}
}
