...
function page(p){
 ...
 $("pageTitle").textContent={dashboard:"Nurse Dashboard",gesture:"Gesture Communication",alerts:"Alert Center",patient:"Patient Care",reports:"Medical Reports",appointments:"Appointments",analytics:"Analytics",faceeye:"Face & Eye AI",mobile:"Mobile App"}[p]||p;
...
async function startFaceCamera(){try{if(faceRunning)return;stopCamera();$("faceStatus").textContent="Starting Face & Eye AI…";if(!window.isSecureContext)throw new Error("Camera requires HTTPS or localhost.");await ensureFaceMesh();const v=$("faceVideo");faceMesh=new window.FaceMesh({locateFile:f=>faceBase+f});faceMesh.setOptions({maxNumFaces:1,refineLandmarks:true,minDetectionConfidence:.55,minTrackingConfidence:.55});faceMesh.onResults(handleFaceResults);faceStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:640},height:{ideal:480}},audio:false});v.srcObject=faceStream;v.muted=true;v.playsInline=true;await v.play();faceRunning=true;faceBusy=false;faceBaselineX=null;faceBaselineY=null;blinkCount=0;eyeClosed=false;primeVoice();$("faceAiBadge").textContent="AI RUNNING";$("faceAiBadge").classList.add("running");$("faceVoiceStatus").textContent="🔊 Automatic multilingual voice is ready";faceFrame=requestAnimationFrame(faceLoop);}catch(e){console.error(e);stopFaceCamera();$("faceStatus").textContent="Face & Eye AI error: "+(e.message||"Could not start");toast(e.message||"Face & Eye AI could not start");}}
