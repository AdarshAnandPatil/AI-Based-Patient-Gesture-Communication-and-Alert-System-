...
function speak(text,l){
 if(!("speechSynthesis"in window))return toast("Voice is not supported on this device");
 speechSynthesis.cancel();
 const u=new SpeechSynthesisUtterance(text);
 u.lang=langs[l||$("languageSelect").value]||"en-IN";u.rate=.9;u.pitch=1;
 const pick=()=>{const voices=speechSynthesis.getVoices();const v=voices.find(x=>x.lang.toLowerCase()===u.lang.toLowerCase())||voices.find(x=>x.lang.toLowerCase().startsWith(u.lang.slice(0,2)));if(v)u.voice=v;speechSynthesis.speak(u)};
 const voices=speechSynthesis.getVoices(); if(voices.length) pick(); else {speechSynthesis.onvoiceschanged=pick;}
 u.onerror=()=>{$("voiceStatus").textContent="⚠️ Voice could not start — tap Start Camera again";toast("Voice failed. Check the device language voice.");};
}
function overlay(a){
 ...
 $("alertOverlay").classList.add("show");speak(`${a.message}. Room ${a.room}. Bed ${a.bed}`,a.language);
}
async function createGestureAlert(number){
 const g=gestureMap[number];if(!g)return;
 const l=$("languageSelect").value,tx=labels[l]||labels.en;
 const message=tx[g.key];
 try{
  const a=await api("/api/alerts",{method:"POST",headers:{"Content-Type":"application/json"},
   body:JSON.stringify({patientId:$("patientId").value,patientName:"Demo Patient",room:$("room").value,bed:$("bed").value,
   gesture:g.name,message,language:l,priority:g.priority,confidence:95})});
  showDetection(number,true);overlay(a);notifyAlert(a);refresh();
 }catch(e){toast(e.message)}
}
function notifyAlert(a){
 if("Notification"in window&&Notification.permission==="granted")new Notification(a.priority==="Critical"?"🚨 Patient Emergency":"Patient Gesture Alert",{body:a.message});
}
function showDetection(number,sent=false){
 const g=gestureMap[number],l=$("languageSelect").value,tx=labels[l]||labels.en;
 if(!g){$("detectedGesture").textContent=tx.waiting;return}
 $("detectedGesture").textContent=`${g.emoji} ${g.name} ${tx.detect}`;
 $("detectedEmoji").textContent=g.emoji;
 $("detectedNeed").textContent=tx[g.key];
 $("detectedDetail").textContent=tx[g.key]==="ALL OK"||tx[g.key]==="ಎಲ್ಲವೂ ಸರಿಯಾಗಿದೆ"||tx[g.key]==="सब ठीक है"?tx.detail:`${tx.detail} ${sent?"✓ Alert sent automatically.":""}`;
 $("voiceStatus").textContent=tx.voiceReady;
 $("detectionPanel").classList.toggle("emergency-detection",g.key==="emergency");
}
function speakDetected(number){
 const g=gestureMap[number],l=$("languageSelect").value,tx=labels[l]||labels.en;if(!g)return;
 const text={food:tx.food,water:tx.water,toilet:tx.toilet,emergency:tx.emergency,ok:tx.ok}[g.key];
 speak(text,l);$("voiceStatus").textContent=`🔊 ${text}`;
}
function updateGuide(){
 ...
 $("guideFood3").textContent=t.food;$("guideFood3Sub").textContent=t.foodSub;
...
function angle(a,b,c){
 ...
 return den?Math.acos(Math.max(-1,Math.min(1,dot/den)))*180/Math.PI:0;
}
function fingerExtended(lm,mcp,pip,tip){
 return angle(lm[mcp],lm[pip],lm[tip])>155 && distance(lm[tip],lm[0])>distance(lm[pip],lm[0])*1.08;
}
function countFingers(lm){
 const ids=[[5,6,8],[9,10,12],[13,14,16],[17,18,20]];
 let count=ids.reduce((n,[m,p,t])=>n+(fingerExtended(lm,m,p,t)?1:0),0);
 const thumb=fingerExtended(lm,2,3,4);
 if(thumb)count++;
 return count;
}

let lastDetected=null,stableCounts=[],lastSentGesture=null,lastSentAt=0,processing=false;
async function handleLandmarks(r){
 const c=$("outputCanvas"),ctx=c.getContext("2d"),v=$("inputVideo");
 c.width=v.videoWidth||640;c.height=v.videoHeight||480;ctx.clearRect(0,0,c.width,c.height);
 const lm=r.multiHandLandmarks?.[0];
 if(!lm){$("cameraStatus").textContent="Camera running — show one hand";$("cameraHint").classList.remove("hidden");stableCounts=[];return}
 $("cameraHint").classList.add("hidden");$("cameraStatus").textContent="✋ Hand detected — counting fingers";
 ctx.lineWidth=3;ctx.strokeStyle="#20d46b";ctx.fillStyle="#20d46b";
 lm.forEach(p=>{ctx.beginPath();ctx.arc(p.x*c.width,p.y*c.height,5,0,Math.PI*2);ctx.fill()});
 const number=countFingers(lm);
 stableCounts.push(number);if(stableCounts.length>8)stableCounts.shift();
 const freq={};stableCounts.forEach(n=>freq[n]=(freq[n]||0)+1);
 const stable=Object.entries(freq).sort((a,b)=>b[1]-a[1])[0];
 if(!stable||Number(stable[1])<6)return;
 const detected=Number(stable[0]);
 if(lastDetected!==detected){lastDetected=detected;showDetection(detected,false)}
 const now=Date.now();
 if(!processing&&(lastSentGesture!==detected||now-lastSentAt>8000)){
  processing=true;lastSentGesture=detected;lastSentAt=now;
  speakDetected(detected);
  await createGestureAlert(detected);
  processing=false;
 }
}
async function startCamera(){
 try{
  if(!window.Hands||!window.Camera)return toast("Camera AI is still loading");
  if(camera)stopCamera();
  const v=$("inputVideo"),hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});
  ...
  camera=new Camera(v,{onFrame:async()=>hands.send({image:v}),width:640,height:480});
  await camera.start();$("cameraStatus").textContent="Camera running — show one hand";$("aiBadge").textContent="AI RUNNING";$("aiBadge").classList.add("running");
  $("voiceStatus").textContent=(labels[$("languageSelect").value]||labels.en).voiceReady;
 }catch(e){toast("Camera permission/device error")}
}
function stopCamera(){
 ...
 stableCounts=[];lastDetected=null;lastSentGesture=null;$("cameraHint").classList.remove("hidden");
