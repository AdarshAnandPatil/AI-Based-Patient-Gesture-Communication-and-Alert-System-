...
async function createMultiAlert(cfg,gesture,message,priority="Normal",confidence=92,voiceText=message){
 ...
 overlay({id:"LOCAL-"+now,...payload,status:"Saving…",createdAt:new Date().toISOString()});
 showPatientEvent(cfg,gesture,message,priority,priority==="Critical"?"🚨":"👤");speak(voiceText,cfg.lang);
 try{const a=await api("/api/alerts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});notifyAlert(a);updateLocalAlert(a);if(activeAlert){activeAlert=a;$("overlayMessage").textContent=a.message;$("overlayMeta").textContent=`Room ${a.room} · Bed ${a.bed}`;$("overlayPatient").textContent=`Patient ${a.patientId} · ${a.patientName||"Patient"}`;$("overlayPriority").textContent="🚨 CRITICAL PATIENT ALERT"}}catch(e){console.warn("Alert save failed",e);toast(`Bed ${cfg.bed}: alert detected, but alert could not be saved.`)}return true;
}
