...
function updateRuntime(id,text){const el=$(id);if(el)el.textContent=text;}
...
function handleLandmarks(r){
 ...
 if(!lm){$("cameraStatus").textContent="Camera running — show one hand";updateRuntime("handLiveStatus","Camera: RUNNING · Waiting for hand…");$("cameraHint").classList.remove("hidden");stableCounts=[];candidateGesture=null;candidateSince=0;if(emergencyModalOpen)closeAlertOverlay();emergencyLatched=false;return}
 $("cameraHint").classList.add("hidden");$("cameraStatus").textContent="✋ Hand detected";setCameraDiagnostic("✓ Hand detected. Processing landmarks…",true);
 ...
 const number=countFingers(lm);updateRuntime("handLiveStatus",`Hand: DETECTED · Fingers: ${number} · Stable samples: ${stableCounts.length}/7`);stableCounts.push(number);if(stableCounts.length>7)stableCounts.shift();
 ...
 showDetection(detected,false);$("detectedDetail").textContent="Gesture confirmed. Speaking and sending alert…";speakDetected(detected);
...
async function startCamera(){
 try{
  ...
  setCameraDiagnostic("Loading MediaPipe Hands model…");
  ...
  try{cameraStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:"user"},width:{ideal:640},height:{ideal:480},frameRate:{ideal:20}},audio:false});}catch(firstError){setCameraDiagnostic("Preferred camera unavailable; trying default camera…");cameraStream=await navigator.mediaDevices.getUserMedia({video:true,audio:false});}
  ...
  $("cameraStatus").textContent="Camera running — show one hand";updateRuntime("handLiveStatus","Camera: RUNNING · Waiting for hand…");$("aiBadge").textContent="AI RUNNING";$("aiBadge").classList.add("running");$("cameraHint").textContent="Show one hand clearly";
...
function stopCamera(){
 ...
 $("cameraStatus").textContent="Camera is off";updateRuntime("handLiveStatus","Camera: OFF");$("aiBadge").textContent="AI READY";$("aiBadge").classList.remove("running");
...
function handleFaceResults(r){faceBusy=false;const lm=r.multiFaceLandmarks?.[0];const t=ft();if(!lm){$("faceStatus").textContent="Camera running — show patient face";$("faceHint").classList.remove("hidden");return;}$("faceHint").classList.add("hidden");$("faceStatus").textContent="🙂 Face detected — eye and head signals active";setFaceDiagnostic("✓ Face detected. Eye and head landmarks are processing.",true);updateRuntime("faceLiveStatus",`Face: DETECTED · Eyes: ${eyeClosed?"CLOSED":"OPEN"} · Blinks: ${blinkCount}`);drawFace(lm);
 ...
 const closed=(left<0.20&&right<0.20);updateRuntime("faceLiveStatus",`Face: DETECTED · Eyes: ${closed?"CLOSED":"OPEN"} · Blinks: ${blinkCount} · EAR: ${((left+right)/2).toFixed(3)}`);
 ...
 if(faceBaselineX===null){faceBaselineX=x;faceBaselineY=y;setFaceDiagnostic("✓ Face baseline calibrated. Keep head centered for one second, then move.",true);}else{ /* Keep a stable baseline; only drift very slowly when near center. */ if(Math.abs(x-faceBaselineX)<.05)faceBaselineX=faceBaselineX*.999+x*.001;if(Math.abs(y-faceBaselineY)<.05)faceBaselineY=faceBaselineY*.999+y*.001;}
