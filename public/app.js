...
async function startCamera(){
 try{if(!window.Hands||!window.Camera)return toast("Camera AI is still loading");if(camera)stopCamera();
  const v=$("inputVideo"),hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});hands.setOptions({maxNumHands:1,modelComplexity:0,minDetectionConfidence:.6,minTrackingConfidence:.6});hands.onResults(handleLandmarks);
  camera=new Camera(v,{onFrame:async()=>hands.send({image:v}),width:640,height:480});await camera.start();$("cameraStatus").textContent="Camera running — show one hand";$("aiBadge").textContent="AI RUNNING";$("aiBadge").classList.add("running");$("voiceStatus").textContent=(labels[$("languageSelect").value]||labels.en).voiceReady;
 }catch(e){console.error(e);toast("Camera permission/device error")}
}
function stopCamera(){if(camera){camera.stop();camera=null}$('inputVideo').srcObject?.getTracks().forEach(t=>t.stop());$("cameraStatus").textContent="Camera is off";$("aiBadge").textContent="AI READY";$("aiBadge").classList.remove("running");stableCounts=[];lastDetected=null;lastSentGesture=null;const l=$("languageSelect").value;$("detectedGesture").textContent=(labels[l]||labels.en).waiting;$("detectedEmoji").textContent="✋";$("detectedNeed").textContent="Show your hand to communicate"}
