...
async function processVideoFrame(now){
 ...
 try{await hands.send({image:v})}catch(e){console.error("Hands frame error",e);}
 finally{processingFrame=false;}
}
async function startCamera(){
 try{
   if(cameraRunning)stopCamera();
   if(!window.Hands){throw new Error("MediaPipe Hands library is still loading")}
   const v=$("inputVideo");
   if(!navigator.mediaDevices?.getUserMedia)throw new Error("Camera API is not available. Use HTTPS/Chrome.");
   ...
   v.srcObject=cameraStream;v.muted=true;await v.play();
   ...
   const msg=e.name==="NotAllowedError"?"Camera permission denied — allow camera in Chrome settings.":e.name==="NotFoundError"?"No camera found on this device.":e.message||"Camera could not start";
   toast(msg);
 }
}
function stopCamera(){
